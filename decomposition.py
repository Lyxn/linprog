...
class SubSystem(object):
    ...
    def solve_sub(self, lmbd, lmbd_i, max_upper=1e16):
        ...
        if type(ret) == int:
            ...
            basis, x_b, r = ret
            self.basis = basis
            #self.x_b = align_basis(x_b, basis, self.col)
            #z_opt = sum(c_sub.take(basis[i]) * x_b[i] for i in range(len(basis))) - lmbd_i
            self.x_b = x_b
...
#    print "Link num\t%s" % row_lnk
    ...
    for itr in range(max_iter):
        ...
        min_sub = 0
        for i in range(n_blk):
            print "Sub-Problem %s" % i
            z_sub = sub_sys[i].solve_sub(lmbd0, lmbd1[i])
